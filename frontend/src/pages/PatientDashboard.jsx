...
function PatientDashboard() {
  ...
  const filteredDoctors = useMemo(() => {
    return doctors.filter((doctor) => {
      ...
      const matchesSpecialty =
        selectedSpecialty === "" ||
        String(doctor.specialty_id) === String(selectedSpecialty);
  ...
  const selectedDoctor = doctors.find(
    (doctor) => String(doctor.doctor_profile_id) === String(selectedDoctorId)
  );

  const selectedDoctorSlots = slots.filter(
    (slot) => String(slot.doctor_profile_id) === String(selectedDoctorId)
  );

  const confirmedConsultations = appointments.filter(
    (appointment) =>
      appointment.status === "CONFIRME" || appointment.status === "TERMINE"
  );

  const pendingAppointments = appointments.filter(
    (appointment) =>
      appointment.status === "EN_ATTENTE" ||
      appointment.status === "EN_ATTENTE_PAIEMENT"
  );
  ...
  const handleBookAppointment = async (event) => {
    ...
    try {
      ...
      setPendingPayment({
        ...
        consultation_price: response.data.consultation_price,
        doctor_first_name: response.data.doctor_first_name,
        doctor_last_name: response.data.doctor_last_name,
      });
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ||
  ...
  const handlePaymentSuccess = async () => {
    ...
    setMessage(
      "Paiement confirmé. Votre rendez-vous a été transmis au médecin."
    );
    await loadDashboard();
        ...
        <main className="dashboard-main">
          <header className="dashboard-header">
            <div>
              ...
              <h1>
                Bonjour, {user.first_name?.trim()} {user.last_name?.trim()}
              </h1>
              ...
              <section className="dashboard-statistics">
                ...
                <article className="dashboard-stat-card">
                  ...
                  <div>
                    ...
                    <strong>{confirmedConsultations.length}</strong>
                  </div>
                </article>
                  ...
                  {nextAppointment && (
                    <div className="appointment-list">
                      <div className="appointment-list-item">
                        <div className="dashboard-user-avatar">
                          {nextAppointment.doctor_first_name?.charAt(0)}
                          {nextAppointment.doctor_last_name?.charAt(0)}
                        </div>

                        <div>
                          <h3>
                            Dr {nextAppointment.doctor_first_name}{" "}
                            {nextAppointment.doctor_last_name}
                          </h3>

                          <p>{nextAppointment.specialty_name}</p>
                  ...
                  <div className="quick-actions">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <small>Voir les médecins disponibles</small>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <small>Consulter mes réservations</small>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <small>Consulter mes prescriptions</small>
                      </div>
                    </button>
                  </div>
                </article>
          ...
          {activeSection === "doctors" && (
            <>
              ...
              <section className="dashboard-grid">
                ...
                {filteredDoctors.map((doctor) => (
                  ...
                  >
                    ...
                    <p>{doctor.years_of_experience} années d’expérience</p>

                    <p>{doctor.biography}</p>

                    <strong>
                      {Number(doctor.consultation_price).toLocaleString(
                        "fr-FR"
                      )}{" "}
                      FCFA
                    </strong>
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleChooseDoctor(doctor.doctor_profile_id)
                      }
                    >
                      Prendre rendez-vous
                    </button>
              ...
              {selectedDoctor && (
                <section className="dashboard-panel" id="booking-form">
                  <div className="dashboard-panel-heading">
                    <div>
                  ...
                  <form onSubmit={handleBookAppointment}>
                    ...
                    {selectedDoctorSlots.length === 0 && (
                      <p>Ce médecin n’a aucun créneau disponible.</p>
                    )}
                    ...
                    <button
                      type="submit"
                      className="primary-dashboard-button"
                      disabled={booking || selectedDoctorSlots.length === 0}
                    >
                      {booking ? "Réservation..." : "Réserver le rendez-vous"}
                    </button>
                  </form>
                </section>
              )}
          ...
          {activeSection === "consultations" && (
            <section className="dashboard-panel">
              ...
              {confirmedConsultations.length === 0 && (
                <p>Vous n’avez aucune consultation confirmée.</p>
              )}

              <div className="appointment-list">
                {confirmedConsultations.map((appointment) => (
                  <div
                    className="appointment-list-item"
                    key={appointment.appointment_id}
                  >
                    <div>
                      ...
                      <p>Statut : {getStatusText(appointment.status)}</p>
                    </div>
